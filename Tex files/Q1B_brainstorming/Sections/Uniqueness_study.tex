...
it is easy to see that at z=-1/a the solution doesnt satisfy the lipshitz condition and is there for non unique at that point. This however is the only point in which the solution breaksdown and is non-unique. Thus, one would conclude that the solution $\input{functions/SOL_1.tex}$ is unique over it's range 

We may then solve for the breakdown of uniqueness in z by setting $z(t) = -\frac{1}{a}$
this gives after a bit of algebra, t = i/a. Thus the solution $\input{functions/SOL_1.tex}$ is unique $\forall\: t \in \mathbb{R}$